...
public:
    explicit OneShotDB(VeryHeavyDatabase* real_object, size_t shots = 1) : real_db_(real_object) { this->shots = shots; }
    std::string GetData(const std::string& key) noexcept {
        if (shots) {
            shots--;
            std::cout << ">>> value";
...
private:
    //std::map<std::string, std::string> ttl;
    size_t shots;
    std::string msg;
    VeryHeavyDatabase* real_db_;
};
...
int main() {
    ...
    auto real_db_os = VeryHeavyDatabase();
    auto limit_db_os = OneShotDB(std::addressof(real_db_os), 2);
    std::cout << limit_db_os.GetData("key") << std::endl;
    std::cout << limit_db_os.GetData("key") << std::endl;
    std::cout << limit_db_os.GetData("key") << std::endl;
